...
int nFieldWidth = 12;
int nFieldHeight = 18;
int nScreenWidth = 80; // Console screen width
...
int main()
{
    ...
    keypad(stdscr, TRUE);
    refresh();
    offsetx = (nScreenWidth - nFieldWidth)/2;
    offsety = (nScreenHeight - nFieldHeight)/2;
    TetrisLand = newwin(nFieldHeight,nFieldWidth,offsety,offsetx);
    refresh();
    ...
    while ((ch = getch()) != 'x') {
		...
		if (!vLines.empty())
		{
			...
			for (int x = 0; x < nFieldWidth; x++)
                for (int y = 0; y < nFieldHeight; y++)
                    mvwaddch(TetrisLand,(y),(x),screen[(y)*nScreenWidth + (x)]);
